...
expiration_date=$(date -d "$(ssh -o StrictHostKeyChecking=no -i key.pem azureuser@$KAFKA_DOMAIN 'sudo openssl x509 -in /etc/letsencrypt/live/$KAFKA_DOMAIN/fullchain.pem -noout -enddate | cut -d= -f2')" +%s)
current_date=$(date +%s)
